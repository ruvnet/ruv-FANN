...
impl PerformanceMonitor {
    ...
    pub fn record_measurement(&mut self, measurement: PerformanceMeasurement) -> Vec<PerformanceAlert> {
    ...
    pub fn get_stats(&mut self, shader_type: &ShaderType, data_size: usize) -> Option<PerformanceStats> {
    ...
    pub fn get_performance_trend(&self, shader_type: &ShaderType, data_size: usize) -> PerformanceTrend {
        let key = (shader_type.clone(), data_size);
    ...
    fn update_real_time_metrics(&mut self, measurement: &PerformanceMeasurement) {
        ...
        self.real_time_metrics.rolling_avg_memory_bandwidth = 
            alpha * measurement.memory_bandwidth_gbps + 
            (1.0 - alpha) * self.real_time_metrics.rolling_avg_memory_bandwidth;
        
        self.real_time_metrics.rolling_avg_compute_throughput = 
            alpha * measurement.compute_throughput_gflops + 
    ...
    fn calculate_stats(&self, measurements: &[PerformanceMeasurement]) -> PerformanceStats {
        ...
        let avg_gpu_occupancy = measurements
            .iter()
            .map(|m| m.gpu_occupancy)
            .sum::<f32>() / count as f32;
    ...
    fn check_performance_alerts(&self, key: &(ShaderType, usize), measurement: &PerformanceMeasurement) -> Vec<PerformanceAlert> {
        let mut alerts = Vec::new();
        ...
        if let Some(measurements) = self.measurements.get(key) {
            if measurements.len() >= 2 {
                ...
                let time_ratio = measurement.execution_time.as_nanos() as f32 / previous.execution_time.as_nanos() as f32;
                if time_ratio > self.alert_thresholds.max_execution_time_degradation {
                    alerts.push(PerformanceAlert {
                        alert_type: AlertType::PerformanceDegradation,
                        severity: if time_ratio > 2.0 { AlertSeverity::Critical } else { AlertSeverity::Warning },
                        shader_type: measurement.shader_type.clone(),
                        data_size: measurement.data_size,
                        message: format!("Execution time increased by {:.1}%", (time_ratio - 1.0) * 100.0),
                        timestamp: measurement.timestamp,
                    });
                }
                
                // Check memory bandwidth
                if measurement.memory_bandwidth_gbps < self.alert_thresholds.min_memory_bandwidth_utilization {
                    alerts.push(PerformanceAlert {
                        alert_type: AlertType::LowMemoryBandwidth,
                        severity: AlertSeverity::Info,
                        shader_type: measurement.shader_type.clone(),
                        data_size: measurement.data_size,
                        message: format!("Low memory bandwidth: {:.1} GB/s", measurement.memory_bandwidth_gbps),
                        timestamp: measurement.timestamp,
                    });
                }
                ...
                if measurement.gpu_occupancy < self.alert_thresholds.min_gpu_occupancy {
                    alerts.push(PerformanceAlert {
                        ...
                        shader_type: measurement.shader_type.clone(),
                        data_size: measurement.data_size,
                        message: format!("Low GPU occupancy: {:.1}%", measurement.gpu_occupancy * 100.0),
                        timestamp: measurement.timestamp,
                    });
                }
            }
