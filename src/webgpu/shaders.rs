...
pub mod embedded {
    ...
    pub const MATRIX_VECTOR_MULTIPLY_SHADER: &str = include_str!("shaders/matrix_vector_multiply.wgsl");
    
    /// Batch matrix-vector multiplication shader
    pub const BATCH_MATRIX_VECTOR_MULTIPLY_SHADER: &str = include_str!("shaders/batch_matrix_vector_multiply.wgsl");
...
pub mod webgpu_shaders {
    ...
    impl ShaderManager {
        ...
        pub fn new() -> Result<Self, ComputeError> {
            ...
            let kernel_optimizer = Arc::new(std::sync::Mutex::new(KernelOptimizer::with_default_capabilities()));
        ...
        pub fn with_gpu_capabilities(gpu_caps: crate::webgpu::kernel_optimizer::GpuCapabilities) -> Result<Self, ComputeError> {
            let pipeline_cache = Arc::new(PipelineCache::new());
        ...
        pub fn get_bind_group_layout(&self, shader_type: &ShaderType) -> Result<Option<()>, ComputeError> {
            match self.pipeline_cache.get_or_create_bind_group_layout(shader_type) {
                Ok(_) => Ok(Some(())),
                Err(e) => Err(e),
            }
        }
        ...
        pub fn get_optimized_config(&self, shader_type: &ShaderType, data_size: usize) -> Result<crate::webgpu::kernel_optimizer::KernelConfig, ComputeError> {
        ...
        pub fn activation_shader_for_function(&self, function: crate::ActivationFunction) -> Option<ShaderType> {
            ...
            match function {
                ...
                ActivationFunction::LinearPieceSymmetric => Some(ShaderType::ActivationLinearPieceSymmetric),
                ActivationFunction::Threshold => Some(ShaderType::ActivationThreshold),
                ActivationFunction::ThresholdSymmetric => Some(ShaderType::ActivationThresholdSymmetric),
            }
        }
        ...
        pub fn get_shader_source(&self, shader_type: &ShaderType) -> Option<&'static str> {
            match shader_type {
                ...
                ShaderType::BatchMatrixVectorMultiply => Some(embedded::BATCH_MATRIX_VECTOR_MULTIPLY_SHADER),
                
                // Activation functions
                ShaderType::ActivationSigmoid | ShaderType::ActivationReLU | ShaderType::ActivationLeakyReLU |
                ShaderType::ActivationTanh | ShaderType::ActivationLinear | ShaderType::ActivationGaussian |
                ShaderType::ActivationGaussianSymmetric | ShaderType::ActivationElliott | ShaderType::ActivationElliottSymmetric |
                ShaderType::ActivationSin | ShaderType::ActivationCos | ShaderType::ActivationSinSymmetric |
                ShaderType::ActivationCosSymmetric | ShaderType::ActivationLinearPiece | ShaderType::ActivationLinearPieceSymmetric |
                ShaderType::ActivationThreshold | ShaderType::ActivationThresholdSymmetric => {
                    Some(embedded::ACTIVATION_FUNCTIONS_SHADER)
                },
                
                // Gradient operations
                ShaderType::GradientSigmoid | ShaderType::GradientReLU | ShaderType::GradientLeakyReLU |
                ShaderType::GradientTanh | ShaderType::GradientLinear | ShaderType::WeightGradient |
                ShaderType::InputGradient | ShaderType::GradientClipping | ShaderType::L2Regularization |
                ShaderType::MomentumUpdate | ShaderType::AdamUpdate | ShaderType::BatchNormGradient => {
                    Some(embedded::GRADIENT_OPERATIONS_SHADER)
                },
                
                // Advanced operations  
                ShaderType::Conv2D | ShaderType::MaxPool2D | ShaderType::AvgPool2D | ShaderType::Softmax |
                ShaderType::LayerNorm | ShaderType::ScaledDotProductAttention | ShaderType::ElementWiseAdd |
                ShaderType::ElementWiseMultiply | ShaderType::ActivationGELU | ShaderType::ActivationSwish => {
                    Some(embedded::ADVANCED_OPERATIONS_SHADER)
                },
        ...
        pub fn get_performance_stats(&self) -> (crate::webgpu::pipeline_cache::CompilationStats, crate::webgpu::pipeline_cache::CacheStats) {
            self.pipeline_cache.get_performance_stats()
        }
        
        /// Record performance metrics for optimization
        pub fn record_performance(&self, shader_type: ShaderType, data_size: usize, metrics: crate::webgpu::kernel_optimizer::OptimizationMetrics) {
            let mut optimizer = self.kernel_optimizer.lock().unwrap();
            optimizer.record_performance(shader_type, data_size, metrics);
        }
        
        /// Get performance predictions
        pub fn predict_performance(&self, shader_type: &ShaderType, data_size: usize) -> Option<crate::webgpu::kernel_optimizer::OptimizationMetrics> {
