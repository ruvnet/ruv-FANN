...
impl KernelOptimizer {
    ...
    pub fn optimize_matrix_vector_multiply(&mut self, rows: usize, cols: usize) -> Result<KernelConfig, ComputeError> {
    ...
    pub fn optimize_batch_matrix_vector(&mut self, rows: usize, cols: usize, batch_size: usize) -> Result<KernelConfig, ComputeError> {
        let cache_key = (ShaderType::BatchMatrixVectorMultiply, rows * cols * batch_size);
    ...
    pub fn optimize_activation_function(&mut self, shader_type: ShaderType, vector_size: usize) -> Result<KernelConfig, ComputeError> {
    ...
    pub fn predict_performance(&self, shader_type: &ShaderType, data_size: usize) -> Option<OptimizationMetrics> {
        ...
        if let Some(history) = self.performance_history.get(&key) {
            if !history.is_empty() {
                ...
                let avg_memory_util = history.iter().map(|m| m.memory_utilization).sum::<f32>() / count;
                let avg_compute_util = history.iter().map(|m| m.compute_utilization).sum::<f32>() / count;
                let avg_occupancy = history.iter().map(|m| m.occupancy).sum::<f32>() / count;
                let avg_memory_eff = history.iter().map(|m| m.memory_efficiency).sum::<f32>() / count;
                let avg_exec_time = history.iter().map(|m| m.estimated_execution_time_us).sum::<f32>() / count;
    ...
    pub fn auto_tune_workgroup_size(&mut self, shader_type: ShaderType, data_size: usize) -> Result<[u32; 3], ComputeError> {
    ...
    fn calculate_matrix_vector_config(&self, rows: usize, _cols: usize) -> Result<KernelConfig, ComputeError> {
    ...
    fn calculate_batch_matrix_vector_config(&self, rows: usize, _cols: usize, batch_size: usize) -> Result<KernelConfig, ComputeError> {
    ...
    fn calculate_activation_config(&self, _shader_type: ShaderType, vector_size: usize) -> Result<KernelConfig, ComputeError> {
    ...
    fn evaluate_workgroup_config(&self, _shader_type: &ShaderType, data_size: usize, workgroup_config: [u32; 3]) -> f32 {
        ...
        let memory_efficiency = if threads_per_workgroup >= 32 { 1.0 } else { threads_per_workgroup as f32 / 32.0 };
        let parallelism_score = (num_workgroups as f32).min(self.gpu_capabilities.compute_units as f32) / self.gpu_capabilities.compute_units as f32;
    ...
    fn estimate_matrix_vector_gflops(&self, rows: usize) -> f32 {
        ...
        let problem_efficiency = if rows >= 1024 { 0.8 } else { rows as f32 / 1024.0 * 0.8 };
...
mod tests {
    ...
    fn test_matrix_vector_optimization() {
        ...
        let config = optimizer.optimize_matrix_vector_multiply(1024, 512).unwrap();
