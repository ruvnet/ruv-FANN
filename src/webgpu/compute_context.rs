...
use crate::webgpu::{
    backend::{ComputeBackend, BackendType, BackendSelector},
...
impl<T: Float + Send + Sync + std::fmt::Debug + 'static> ComputeContext<T> {
    ...
    pub fn select_backend(&mut self, problem_size: usize) -> BackendType {
        ...
        let selected = self.backend_selector.select_backend(&profile)
    ...
    fn get_layer_weights(&mut self, layer: &Layer<T>, layer_id: usize) -> ComputeResult<(Vec<T>, MatrixDims)> {
        ...
        if input_size == 0 || output_size == 0 {
            return Err(ComputeError::InvalidDimensions(
                format!("Invalid layer dimensions: {}x{}", output_size, input_size)
            ));
        }
        ...
        T: Clone + num_traits::ToPrimitive + 'static,
    {
        #[cfg(feature = "webgpu")]
        {
            if let Some(ref gpu_backend) = self.webgpu_backend {
                ...
                let outputs = gpu_backend.matrix_vector_multiply(weights, inputs, dims.rows, dims.cols)?;
    ...
    async fn compute_layer_cpu(
        ...
        let activation_function = layer.neurons.iter()
            ...
            .map(|n| n.activation_function)
            .unwrap_or(ActivationFunction::Linear);
        let result: Vec<T> = outputs.into_iter()
...
impl PerformanceTracker {
    ...
    fn get_stats(&self) -> TrackerStats {
        TrackerStats {
            ...
            average_duration: self.execution_times.values()
                .flat_map(|times| times.iter())
                .sum::<f64>() / self.execution_times.values()
                .map(|times| times.len())
                .sum::<usize>() as f64,
